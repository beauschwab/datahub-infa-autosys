...
app = typer.Typer(
    ...
    help="DataHub ingestion helpers — run recipes, extract, and inspect metadata sources.",
)
...
def ingest(
    ...
    typer.echo(f"✔ Pipeline complete — source={pipeline.config.source.type}")
...
def extract_informatica(
    ...
    typer.echo(f"Exported → {xml_path}")
